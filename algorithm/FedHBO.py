...
#         self.init_algo_para({'mu': 0.1})  #mu为超参数，设定为0.1
#
...
#             # 初始化相关参数
#             def objf(x): #定义目标函数
#                 fitness_value =  0.01 * np.exp(x) + 0.495 * 1 / (1+np.exp(x))
...
#
# # 使用示例
# solution = Solution(optimizer_name='bhbo')
# optimizer = solution.get_optimizer(model=None)

##
# 上述修改将 BHBO 作为一种优化器，当 optimizer_name 参数为 'bhbo' 时，会创建 BHBO 优化器对象，并返回。
# 其他情况下，仍然会根据输入的优化器名称创建对应的 PyTorch 优化器对象。
# #
...
class Client(BasicClient):
    ...
    def train(self, model):
        ...
        src_model.freeze_grad()
        model.train()
        optimizer = self.calculator.get_optimizer(BHBO)
        for iter in range(self.num_steps):
            batch_data = self.get_batch_data()
            model.zero_grad()
            loss = self.calculator.compute_loss(model, batch_data)['loss']
